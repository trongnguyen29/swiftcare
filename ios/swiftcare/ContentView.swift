...
struct ContentView: View {
            ...
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .leading) {
                    ...
                    Color.black.opacity(0.28 * effectiveDrawerProgress)
                        ...
                        .onTapGesture { withAnimation(.spring(response: 0.35, dampingFraction: 0.86)) { sidebarOpen = false } }
    ...
    // MARK: - Patients page (detail + hamburger)
...
struct NativeGlassTabBar: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITabBar {
        ...
        tabBar.items = AppTab.allCases.map { appTab in
            UITabBarItem(
                title: appTab.title,
                image: UIImage(systemName: appTab.icon),
                tag: appTab.rawValue
            )
        }
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(tab: $tab, onSelect: onSelect)
    }
